...
from keras.layers import Flatten,Reshape,Concatenate,MaxPool1D,LSTM
...
ax.set_title('Confusion Matrix of BiLSTM on SARC'); 
...
# labels, title and ticks
ax.set_xlabel('Predicted labels');ax.set_ylabel('True labels'); 
ax.set_title('Confusion Matrix of BiLSTM on Amazon'); 
ax.xaxis.set_ticklabels(['Non - Sarcastic', 'Sarcastic']); ax.yaxis.set_ticklabels(['Non - Sarcastic', 'Sarcastic']);
# accuracy: (tp + tn) / (p + n)
