...
mc = ModelCheckpoint('/content/drive/My Drive/Sarcasm_bestModel/CNN_LSTM_Glove_best.h5', monitor='val_accuracy', mode='max', verbose=1, save_best_only=True)
...
ax.set_title('Confusion Matrix of CNN - LSTM on SARC'); 
...
# labels, title and ticks
ax.set_xlabel('Predicted labels');ax.set_ylabel('True labels'); 
ax.set_title('Confusion Matrix of CNN - LSTM'); 
ax.xaxis.set_ticklabels(['Non - Sarcastic', 'Sarcastic']); ax.yaxis.set_ticklabels(['Non - Sarcastic', 'Sarcastic']);
# accuracy: (tp + tn) / (p + n)
